...
locals {
  ...
  # タスク定義ファミリー名
  task_definition_family = var.task_definition_family != "" ? var.task_definition_family : "${var.project}-${var.env}-${var.app}"

  # サービス名
  service_name = var.service_name != "" ? var.service_name : "${var.project}-${var.env}-${var.app}-service"

  # コンテナ名
  container_name = var.container_name != "" ? var.container_name : "${var.project}-${var.env}-${var.app}"
